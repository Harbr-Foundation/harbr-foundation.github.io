  ...
  let { data } = $props();
    ...
    const popularArticles = [
      { 
        ...
        slug: 'quick-start',
        icon: PlayCircle,
        description: 'Get up and running with Harbr in under 5 minutes'
      },
      {
        title: 'Authentication Setup',
        slug: 'authentication',
        icon: KeyRound,
        description: 'Learn how to secure your repositories and manage access'
      },
      {
        title: 'Advanced Features',
        slug: 'advanced-features',
        icon: Star,
        description: 'Explore powerful features for large-scale development'
      },
      {
        title: 'CLI Reference',
        slug: 'cli-reference',
      ...
      <div class="mx-auto max-w-7xl px-6 lg:px-8">
        <div class="mx-auto max-w-2xl text-center">
          ...
          <p class="text-lg leading-8 text-zinc-400">
            Learn everything about Harbr's powerful version control features.
          </p>
        </div>
        ...
        <div class="mx-auto mt-8 max-w-2xl">
          <div class="relative group">
            ...
            <div class="hidden lg:flex absolute right-4 top-3.5 items-center gap-2">
              <kbd class="px-2 py-1 text-xs rounded bg-zinc-800 text-zinc-400 border border-zinc-700">
                ⌘
              </kbd>
              <kbd class="px-2 py-1 text-xs rounded bg-zinc-800 text-zinc-400 border border-zinc-700">
                K
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {#each popularArticles as article}
            ...
            >
              ...
              <ArrowRight 
                size={16} 
                class="absolute bottom-6 right-6 text-zinc-600 group-hover:text-emerald-500 
                       transition-colors" 
              />
            </a>
          {/each}
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3" in:fade>
          {#each filteredDocs as doc}
            ...
            >
              ...
              <div>
                {#if doc.category}
                  <span class="text-xs font-medium text-emerald-500 mb-2 block">
                    {doc.category}
                  </span>
                {/if}
                <h3 class="text-lg font-medium text-white group-hover:text-emerald-500 
              ...
              {#if doc.tags?.length}
                <div class="mt-4 flex flex-wrap gap-2">
                  {#each doc.tags as tag}
                    <span class="px-2 py-0.5 text-xs rounded-full bg-emerald-500/10 text-emerald-500">
                      {tag}
                    </span>
                  {/each}
                </div>
